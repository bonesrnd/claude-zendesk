...
pnpm --filter @resolve/orchestrator exec wrangler secret put WOO_BASE_URL
```
...
`WOO_BASE_URL` is the pinned HTTPS WooCommerce origin. Enter the same origin in the Zendesk visible setting. The Worker rejects a mismatch before forwarding WooCommerce credentials, preventing an agent-controlled header from redirecting secrets.

## 4. Provision and migrate D1

The D1 binding in `wrangler.jsonc` uses automatic resource provisioning. Confirm the binding during a dry run, then deploy and apply the remote migration:
...
- WooCommerce store URL: HTTPS store origin.
...
3. Run one WooCommerce lookup.
4. Run one ShipStation lookup for every configured API generation.
5. Retrieve one prior Zendesk ticket.
6. Open every returned citation.
7. Reload as another authorized agent and confirm history persists.
8. Invalidate one test credential and confirm Resolve reports a safe configuration error without showing the value.
9. Inspect Worker logs for request IDs, durations, statuses, and no message bodies or credentials.
