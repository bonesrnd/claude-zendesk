...
9. Reload the ticket and verify saved messages.
10. Open the same ticket as another authorized agent and verify team visibility.
11. Verify an ambiguous search asks for a selection rather than choosing silently.
12. Verify one unavailable provider produces a partial, clearly labeled result.
13. Verify malformed model or effort settings produce a safe configuration error.
14. Verify Worker logs contain no credentials, prompts, or customer messages.
